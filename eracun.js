...
streznik.get('/', function(zahteva, odgovor) {
  if (zahteva.session.atm == undefined)
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    ...
    try {
      var stmt = pb.prepare("\
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
      stmt.run(polja.FirstName, polja.LastName, polja.CompileStep, polja.Address, polja.City, polja.State, polja.Country, 
...
streznik.post('/stranka', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    zahteva.session.atm = polja.seznamStrank;
    odgovor.redirect('/')
  });
})
